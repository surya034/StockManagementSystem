...
def connection():
    conn = pymysql.connect(
        host="localhost", user="root", password="Surya@2002", db="project"
    )
    return conn
...
def save():
    ...
    valid = True
    if (
        not (itemId and itemId.strip())
        or not (name and name.strip())
        or not (price and price.strip())
        or not (qnt and qnt.strip())
        or not (cat and cat.strip())
    ):
...
categoryCombo = ttk.Combobox(
    entriesFrame, width=47, textvariable=placeholderArray[4], values=categoryArray
)
